...
/**
 * Declares and defines one or more global or local variables. Consists of declaration specifiers
 * (giving the base type) and list of one or more init-declarators. Each init-declarator has
 * a declarator (modifies the base type and gives a variable name), and an optional initializer
 * expression. For global variables the initializer must be a compile time constant expression.
 *
 * <br> EBNF definition:
 *
 * <br> VARIABLE_DECLARATION = DECLARATION_SPECIFIERS INIT_DECLARATOR_LIST ";"
 *
...
public class VariableDeclaration extends Declaration
{
	 ...
	 */
	public VariableDeclaration(DeclarationSpecifiers declarationSpecifiers,
		...
		if (declSpecifiers != null) {
			...
			if (initDeclList != null) {
				if (tokens.read().toString().equals(";"))
					varDeclaration = new VariableDeclaration(declSpecifiers, initDeclList, pos);
			}
		}
